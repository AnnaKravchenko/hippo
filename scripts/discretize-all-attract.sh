# this one handles coordinated made directly from .dat
# it's mostly about the naming convention
# now it's not a priority to merge both versios in one
...
for i in ???-*.npy; do 
    ...
    echo $motif $atomtype
    mkdir -p cache-$motif
    #echo " $i $atomtype $histo_dir/1-$atomtype.json cache-$motif"
